...
```{=tex}
Using the Chebychev's Inequality, that says, for any $k > 0$ we have $$P(|X-\mu| \geq k \sigma) \leq \frac{1}{k^{2}}$$ Since, we got k = 8, from Chebychev's Inequality, we get $P(|X-\mu| \geq 8 \sigma) \leq \frac{1}{8^{2}} = 0.015625$. This means that the probability to observe a value as large as 9.2661 is less than 1.56 \%. Hence we can conclude that the two distributions in the underlying population are different.
```
