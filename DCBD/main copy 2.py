...
# secret_word = '[^a-zA-Z0-9$#!&<>?\^]'
# Intervals of text that we will keep
intervals = []
interval_size = 50 # additional characters added on both sided of pattern
...
keyVerbs = ['\bis\b', '\bare\b', '\bwas\b', '\bwere\b', '\bbe\b', '\bbeen\b', '\bbeing\b', '\bam\b', '\baren',
'wasn\b', 'weren\b','\bi\b', '\byou\b', '\bhe\b', '\bshe\b', '\bit\b', '\bwe\b', '\bthey\b', '\bme\b', '\byou\b',
'\bhim\b', '\bher\b', '\bus\b', '\bthem\b', '\bmy\b', '\byour\b', '\bhis\b', '\bher\b', '\bits\b', '\bour\b',
'\btheir\b', '\bmine\b', '\byours\b', '\bhers\b', '\bours\b', '\btheirs\b','\bvery\b', '\breally\b',
'\bextremely\b', '\bquite\b', '\bso\b', '\btoo\b', '\bjust\b', '\benough\b', '\balmost\b', '\beven\b',
'\bnearly\b','\ba\b', '\ban\b','\bhome\b', '\babout\b', '\bproducts\b', '\bcontact\b', '\bblog\b',
'\bnews\b', '\bevents\b', '\bgallery\b', '\bcareers\b', '\bfaq\b', '\bterms\b', '\bprivacy\b',
'\bsearch\b', '\blogin\b', '\bregister\b', '\bcart\b', '\bcheckout\b', '\bprofile\b', '\blogout\b',
'\bproduct\b', '\bhave\b', '\bhas\b', '\bhad\b', '\bhaving\b', '\bgo\b', '\bgoing\b','\bdo\b','\bdoing\b',
'\border\b', '\bpayment\b', '\bbill\b', '\bpaid\b', '\bdone\b', '\bpaying\b', '\btherefore\b', '\bthus\b',
'\bhence\b', '\bwhat\b', '\bwhen\b', '\bwho\b', '\bwhom\b', '\bhow\b', '\bwhy\b', '\bnot\b', '\bkind\b', '\bwith\b',
'\bwithout\b', '\bwebsite\b', '\be-mail\b', '\bemail\b', '\bbecause\b', '\bdue\b', '\ball\b', '\bmany\b',
'\bfew\b', '\bsome\b', '\bquite\b', '\blike\b', '\baddress\b', '\bextreme\b','\bfor\b']
...
patternVerbsStr = "|".join(keyVerbs)
patternVerbsStr = r'{}'.format(patternVerbsStr)

patternVerbs = re.compile(patternVerbsStr,flags=re.I)
...
for x in range(10):
    ...
    if (file.exists()):
        ...
        output = re.sub('\n ', '\n', output)
        output = re.sub(' \n', '\n', output)
        output = re.sub(patternVerbs, '', output)
        print(output)
        #-------Layer 2----------
